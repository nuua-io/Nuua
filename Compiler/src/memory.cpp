...
void Memory::dump()
{
    ...
    for (size_t i = 0; i < this->code->size(); i++) {
        // Get the opcode
        opcode_t opcode = this->code->at(i);
        // Print the opcode
        print_opcode(opcode);
        ...
        for (uint8_t k = 0; k < 3 && operands->at(k) != OT_NONE; k++) {
            switch (operands->at(k)) {
                case OT_REG: { printf(" R%04d", this->code->at(++i)); break; }
                case OT_CONST: { printf(" C%04d", this->code->at(++i)); break; }
                case OT_GLOBAL: { printf(" G%04d", this->code->at(++i)); break; }
                case OT_LITERAL: { printf(" %05d", this->code->at(++i)); break; }
