...
#define READ_CONSTANT() (new Value( *this->get_current_memory()->constants.at(READ_INSTRUCTION()) ))
...
void VirtualMachine::do_dictionary()
{
    ...
    PUSH(new Value(dictionary, key_order));
}
...
void VirtualMachine::run()
{
    for (uint64_t instruction;;) {
        ...
        switch (instruction) {
            ...
            case OP_LOAD: { PUSH(new Value(*this->top_frame->heap.at(READ_VARIABLE()))); break; }
            case OP_LIST: { this->do_list(); break; }
            case OP_DICTIONARY: { this->do_dictionary(); break; }
            case OP_ACCESS: { this->do_access(); break; }
            case OP_FUNCTION: { PUSH(new Value(READ_INTEGER(), new Frame(*this->top_frame))); break; }
            case OP_CALL: { break; }
